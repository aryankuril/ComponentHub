...
export default function ComponentForm({ onSuccess, initialData = null }: { onSuccess: () => void; initialData?: any }) {
  ...
  // Use a custom state setter to handle message changes more safely
  const setMessage = (msg: string, isSuccess = false) => {
    const color = isSuccess ? 'text-green-500' : 'text-red-500';
    useStateMessage(<p className={`text-sm text-center ${color}`}>{msg}</p>);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    useStateMessage(''); // Clear previous messages
    ...
    try {
      ...
      if (res.ok) {
        ...
        onSuccess();
        // Only reset form state if it's a new component being created
        if (!initialData) {
          setName('');
          setDescription('');
          setCode('');
    ...
    <form onSubmit={handleSubmit} className="space-y-6">
      ...
      <div className="flex space-x-4">
        <div className="flex-1 space-y-4">
          ...
          <div>
            <label className="block text-sm font-medium mb-1">Required npm Packages (comma-separated)</label>
            <input
              type="text"
              value={npmPackages}
        ...
        <div className="flex-1">
          <div>
            <label className="block text-sm font-medium mb-1">Component Code (Next.js + TailwindCSS)</label>
            <textarea
              value={code}
