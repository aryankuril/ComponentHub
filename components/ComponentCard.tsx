...
<div className="aspect-video bg-[#060606] relative overflow-hidden">
        ...
        {component.previewImage ? (
          ...
          <div className="w-full h-full flex items-center justify-center">
            <div className="w-16 h-16 bg-gradient-neon rounded-lg animate-pulse" />
          </div>
        )}
        ...
        <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all duration-300">
          <button
            onClick={onPreview}
            className="bg-neon-cyan text-black px-4 py-2 rounded-md font-semibold transform scale-95 group-hover:scale-100 transition-transform duration-300 shadow-[0_0_15px_hsl(180,100%,50%)]"
          >
        ...
        <div className="flex items-center justify-between text-xs text-muted-foreground">
          ...
          {component.dateCreated && (
            <div className="flex items-center space-x-1 text-gray-400">
              ...
              <span>{new Date(component.dateCreated).toLocaleDateString()}</span>
            </div>
          )}
        </div>
      </div>
      ...
      <div className="p-6 pt-0">
        <Link
          ...
          className="block text-center mt-4 border-2 border-neon-cyan text-neon-cyan rounded-full px-4 py-2 font-semibold hover:bg-neon-cyan hover:text-black transition-all duration-300 "
        >
          View Component
        </Link>
      </div>
